...
function addQuestion(form, question) {
  ...
  var itemChoices = question.choices.map(choice => item.createChoice(choice, choice === question.answer));
  item.setChoices(itemChoices);
  item.setPoints(1).setTitle(question.title).setHelpText(question.unit).setRequired(true);
}
...
function generateGoogleForm(questions, options) {
  ...
  const datetime = Utilities.formatDate(new Date(), 'GMT+8', "yyyy-MM-dd HH:mm:ss");
  console.log(`created form @${datetime}`);
  var form = FormApp.create(`LS Concept`);
  form.setDescription(`於${datetime}生成。[${options.units.join(', ')}]。共 ${questions.length} 題。`);
